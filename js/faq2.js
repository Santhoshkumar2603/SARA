var firebaseConfig = {
  ...
  firebase.auth().onAuthStateChanged((user) => {
    if (user) {
      ...
      var first = snap.child("/31/1").val();
      var second = snap.child("/32/2").val();
      var thirs = snap.child("/23/3").val();
      var fourth = snap.child("/24/4").val();
      var five = snap.child("/25/5").val();
      console.log(second)
  
  let labels2 = ['Self Assessment', 'Typing Assessment'];
  ...
  $(document).ready(function(){
      ...
      <div class="d-flex w-100 justify-content-between">
        <h5><b>${element.question3}</b></h5>
      </div>
      <br>
      <p style="color:#0a2166" class="mb-1"> ${element.answer3}</p>
      </div>`;
        }
