...
class Game {
	...
	initUI() {
		...
		for(var n=0; n<15; n++) {
			var snd = new Audio();
				snd.src = "aud/pluck." + ( snd.canPlayType("audio/mpeg") ? "mp3" : "ogg" );
	...
	levelUp() {
		if(this.round.level==3)
		{
			...
			swal("Wow!! You Accuracy is", ((this.round.accuracy / this.round.word_count) * 100).toFixed(1) + "%", "success");
			var san=((this.round.accuracy / this.round.word_count) * 100).toFixed(1);
			//alert(((this.round.accuracy / this.round.word_count) * 100).toFixed(1) + "%");
